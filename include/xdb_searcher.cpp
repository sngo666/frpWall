...
XDB_PRIVATE(int)
read(xdb_searcher_t *, long offset, char *, size_t length);

XDB_PRIVATE(int)
xdb_new_base(xdb_searcher_t *xdb, const char *db_path, const xdb_vector_index_t *v_index, const xdb_content_t *c_buffer)
{
...
XDB_PUBLIC(int)
xdb_new_with_file_only(xdb_searcher_t *xdb, const char *db_path)
{
  return xdb_new_base(xdb, db_path, NULL, NULL);
}

XDB_PUBLIC(int)
xdb_new_with_vector_index(xdb_searcher_t *xdb, const char *db_path, const xdb_vector_index_t *v_index)
{
  return xdb_new_base(xdb, db_path, v_index, NULL);
}

XDB_PUBLIC(int)
xdb_new_with_buffer(xdb_searcher_t *xdb, const xdb_content_t *c_buffer)
{
...
XDB_PUBLIC(int)
xdb_search_by_string(xdb_searcher_t *xdb, const char *str_ip, char *region_buffer, size_t length)
{
  unsigned int ip = 0;
...
XDB_PUBLIC(int)
xdb_search(xdb_searcher_t *xdb, unsigned int ip, char *region_buffer, size_t length)
{
...
XDB_PRIVATE(int)
read(xdb_searcher_t *xdb, long offset, char *buffer, size_t length)
{
...
XDB_PUBLIC(int)
xdb_get_io_count(xdb_searcher_t *xdb)
{
  return xdb->io_count;
}
...
XDB_PUBLIC(xdb_header_t *)
xdb_load_header(FILE *handle)
{
  xdb_header_t *header;
...
XDB_PUBLIC(xdb_header_t *)
xdb_load_header_from_file(const char *db_path)
{
  xdb_header_t *header;
  FILE *handle = fopen(db_path, "rb");
  if (handle == NULL)
...
XDB_PUBLIC(xdb_vector_index_t *)
xdb_load_vector_index(FILE *handle)
{
...
XDB_PUBLIC(xdb_vector_index_t *)
xdb_load_vector_index_from_file(const char *db_path)
{
  xdb_vector_index_t *v_index;
  FILE *handle = fopen(db_path, "rb");
  if (handle == NULL)
...
XDB_PUBLIC(void)
xdb_close_vector_index(void *ptr)
{
...
XDB_PUBLIC(xdb_content_t *)
xdb_load_content(FILE *handle)
{
  unsigned int size;
...
XDB_PUBLIC(xdb_content_t *)
xdb_load_content_from_file(const char *db_path)
{
  xdb_content_t *content;
  FILE *handle = fopen(db_path, "rb");
  if (handle == NULL)
...
XDB_PUBLIC(unsigned int)
xdb_get_uint(const char *buffer, int offset)
{
  return (
...
XDB_PUBLIC(int)
xdb_get_ushort(const char *buffer, int offset)
{
  return (
      ((buffer[offset]) & 0x000000FF) |
      ((buffer[offset + 1] << 8) & 0x0000FF00));
...
XDB_PUBLIC(int)
xdb_check_ip(const char *src_ip, unsigned int *dst_ip)
{
  char c;
  int i, n, ip = 0;
...
XDB_PUBLIC(void)
xdb_long2ip(unsigned int ip, char *buffer)
{
...
XDB_PUBLIC(unsigned int)
xdb_mip(unsigned long a, unsigned long b)
{
